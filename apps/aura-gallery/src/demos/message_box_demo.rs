use aura_components::{alert, confirm, Button, MessageBox, Space};
...
impl Render for MessageBoxDemo {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div().flex().flex_col().gap_6()
            ...
            .child(section(theme, "Close strategy 关闭策略", "可禁用空白处和 ESC 关闭。"))
            .child(
                Space::new().gap(px(16.0))
                    .child(Button::new("Manual Alert").warning().on_click(|_, _, cx| {
                        MessageBox::new("Manual Alert", "Only the OK button can close this message box.")
                            .close_on_click_outside(false)
                            .close_on_escape(false)
                            .alert(cx);
                    }))
                    .child(Button::new("Manual Confirm").danger().on_click(|_, _, cx| {
                        MessageBox::new("Manual Confirm", "Only Cancel or Confirm can close this message box.")
                            .close_on_click_outside(false)
                            .close_on_escape(false)
