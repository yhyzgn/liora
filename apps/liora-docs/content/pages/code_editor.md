...
`CodeEditor` 是 Liora 的原生代码编辑控件基础版，面向配置面板、低代码脚本、小型规则编辑器和文档示例编辑场景。它保持纯 Rust + GPUI native：编辑核心复用 Liora `Input`，高亮预览复用 `CodeBlock` 的 `syntect` / `two-face` 资源，诊断信息通过数据结构注入，不绑定 WebView、Monaco 或浏览器运行时。
...
::LioraDemo{component="CodeEditorAdvanced"}::
