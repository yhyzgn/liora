...
- **Application**：`gpui::Application::new().run(...)` 启动原生事件循环。所有窗口和全局状态都在这个循环里创建。
...
Liora 使用 Zed 官方 crates.io `gpui`。外部项目可以按下面方式配置：
...
- `gpui` 是 Zed 官方 crates.io 包，repository 指向 `https://github.com/zed-industries/zed`。不要使用非官方 fork。
