...
Liora 使用 Zed 官方 `zed-industries/zed` git 依赖，并通过 `gpui_platform::application()` 启动原生事件循环。外部项目可以按下面方式配置：
...
- `gpui` 与 `gpui_platform` 都来自 Zed 官方 `https://github.com/zed-industries/zed`，并应 pin 到同一个 `rev`。不要使用非官方 fork。
...
- 字体默认走系统：如需自定义 UI 或代码字体，应用先用 `liora::load_custom_fonts(cx, ...)` 注册字体字节，再用 `LioraOptions::system().with_fonts(FontConfig::system().with_ui_family(...).with_code_family(...))` 初始化，或运行时调用 `liora::set_font_config(cx, ...)`。
