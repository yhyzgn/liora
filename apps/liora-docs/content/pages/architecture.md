...
发布 SDK 还有一条额外边界：`liora`、`liora-core`、`liora-components`、`liora-packager` 等 crates.io 包必须依赖 crates.io `open-gpui` / `open-gpui-platform`，不能包含 `[patch.crates-io]`，也不能把 `third_party/zed` path 依赖写入可发布 manifest。Cargo patch 是应用根 package 的解析决策；库 crate 不能可靠地替下游应用选择 GPUI backend。

本仓库保留 `third_party/zed` 只用于本地 Gallery / Docs 首帧窗口状态验证和 upstream PR 对照。这个目录中的包名保持 upstream `gpui` / `gpui_linux`，因此不能直接作为 `open-gpui` / `open-gpui-linux` 的 crates.io patch。需要验证该 patch 时，临时分支应修改根 `[workspace.dependencies]` alias，让所有 Liora crates 一起改用 Zed git `gpui` / `gpui_platform` 依赖，并用 `[patch."https://github.com/zed-industries/zed"]` 指向 `third_party/zed/crates/gpui` 与 `third_party/zed/crates/gpui_linux`；或者使用包名与 `open-gpui` 系列一致的私有 fork。该 override 不进入 SDK 发布物。

这也是外部项目接入 Liora 时应遵守的策略：平台 feature 与 GPUI backend override 都由最终应用显式选择，Liora SDK 只提供兼容的组件与初始化 API。
