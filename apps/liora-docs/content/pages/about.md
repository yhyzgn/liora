# About Liora

Liora UI 是一个探索 Rust 原生桌面 UI 的组件库项目。它的目标不是复刻网页组件库，而是在 GPUI 的原生窗口模型上沉淀一套适合企业级工具、后台系统、数据密集应用和开发者工具的 UI 基础设施。

## 项目愿景

Liora 希望证明：复杂文档、组件展示、表单、弹层、消息反馈、代码阅读和图片预览都可以在纯 Rust 原生应用里完成，而不必退回 WebView。

## 当前重点

- 建立稳定的组件 API。
- 完善 Docs，每个效果都配套可检查代码。
- 优化交互性能，例如代码块选择、图片远程加载、Preview 关闭行为。
- 补齐动画基础设施，使 Button、Switch、Loading、Popover、Dialog、Drawer 等组件拥有一致动效。
- 保持 Gallery 和 Docs 双主程序互相验证。

## 导航与示例策略

Gallery 和 Docs 的左侧导航保持三段式结构：

1. `About` 固定在第一组，承载项目定位、更新入口、贡献规则、维护约定，以及 Overview、Quick Start、Architecture、Packaging、Release、Adoption、Dashboard、Authoring 等非控件/非布局类说明。
2. `窗体布局` 放置真正参与窗口、应用框架、布局、浮层和侧边栏结构的组件，例如 Shell、TitleBar、Sidebar、Container、Dialog、Drawer、Popover、Tray。
3. `控件` 放置所有可复用 UI 控件、数据展示、图表、反馈和编辑工具。

这样做的目标是让新读者先看到项目边界与采用说明，再按“搭应用壳 / 找控件”的方式扫描。Docs 中的技术说明、流程说明和声明类页面统一归入 About，不混进窗体布局；随着控件数量增长，不再继续拆成大量细分类，避免左侧目录变成难以搜索的 taxonomy。

Gallery 示例应优先使用 Liora 组件、layout helpers 和公开扩展点。除应用入口、prelude、unit 等必要上下文外，示例布局不应直接写 GPUI 原生布局代码；如果某个场景必须绕过封装，优先增强 SDK 控件能力，而不是在应用层堆临时代码。

## 贡献文档的规则

新增或修改组件文档时，请遵守：

1. Markdown 页面放在 `apps/liora-docs/content/pages`。
2. Rust 代码片段放在 `apps/liora-docs/content/snippets/<component>`。
3. 每个效果章节必须紧跟对应代码。
4. `.rs` snippet 应包含必要 imports 和注释，并通过 `check_snippets` 编译检查。
5. Live Demo key 需要在 Docs renderer 中映射到真实组件。

## 非目标

Liora 不计划提供 HTML 输出、不计划运行在浏览器里，也不把 CSS 作为样式系统。样式、布局和交互都应通过 Rust API、GPUI 元素和 Liora theme token 表达。


## About 栏与自动更新

Gallery 和 Docs 都提供 About / 关于入口。应用启动后会自动检查 GitHub Releases 是否存在新版本，并把通过校验的资产下载到本地更新缓存；About 面板可以继续手动触发检查、下载和安装计划。

`liora-updater` 不是 Gallery/Docs 专用模块，而是可复用的 GitHub Release updater SDK：应用必须自己配置 owner/repo、当前版本、asset selector、缓存目录、checksum asset 名称和安装器偏好。Gallery 和 Docs 只是在应用层各自定义本地 selector，用于选择 `liora-gallery` / `liora-docs` 的当前平台资产；SDK 内不保留任何官方应用预设。

为了避免静默提权或替换运行中的程序，Liora 不在后台偷偷执行系统级安装。Gallery 对安装器资产（例如 AppImage、DMG、NSIS/MSI）提供可执行安装动作；Docs 当前按照发布策略下载原始可执行文件，并给出手动替换计划。
