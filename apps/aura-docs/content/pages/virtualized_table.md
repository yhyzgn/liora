...
`VirtualizedTable` 是面向大数据行的原生虚拟表格：表头固定在容器顶部，表体由 GPUI `ListState` 只渲染可见行，并通过 Aura `VirtualScrollbar` 显示滚动位置。
...
::AuraDemo{component="VirtualizedTableBasic"}::
...
::AuraDemo{component="VirtualizedTableSortable"}::
