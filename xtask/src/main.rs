...
impl ReleaseReadinessReport {
    ...
    fn to_markdown(&self) -> String {
        let mut text = String::from("# Aura release readiness report\n\n");
        text.push_str(
...
fn check_license_policy(root: &Path, report: &mut ReleaseReadinessReport) {
    ...
    match fs::read_to_string(&license_path) {
        Ok(text) if text.contains("LicenseRef-Aura") => report.pass(
            "license policy",
            "LICENSE.md declares the repository's current LicenseRef-Aura proprietary policy",
        ),
        Ok(_) => report.fail(
            "license policy",
            "LICENSE.md exists but does not declare the LicenseRef-Aura package policy",
        ),
        Err(_) => report.fail(
            "license policy",
            "missing LICENSE.md; choose OSS license metadata or document LicenseRef-Aura proprietary policy before release",
        ),
    }
}
        ...
        if let Some(tag_version) = ref_name.strip_prefix('v') {
            if tag_version == base_version && tag_version == version {
                ...
                report.fail(
                    "release tag version",
                    format!(
                        "tag {ref_name} must match crates/aura-packager version {base_version} and AURA_PACKAGE_VERSION {version}"
                    ),
                );
            }
            return;
...
fn check_signing_policy(report: &mut ReleaseReadinessReport) {
    let require_signing = env_truthy("AURA_REQUIRE_SIGNING");
    ...
    if missing.is_empty() {
        ...
        report.fail(
            "signing/notarization policy",
            format!(
                "AURA_REQUIRE_SIGNING=true but missing {}",
                missing.join(", ")
            ),
        );
    } else {
        report.warn(
            "signing/notarization policy",
            format!(
                "unsigned build allowed; set AURA_REQUIRE_SIGNING=true and provide {} for signed release enforcement",
                missing.join(", ")
            ),
        );
    }
...
fn signing_env_missing(platform: Platform) -> Vec<&'static str> {
    let required = match platform {
        Platform::Linux => &[][..],
        Platform::Macos => &[
            "AURA_MACOS_CODESIGN_IDENTITY",
            "AURA_MACOS_NOTARY_APPLE_ID",
            "AURA_MACOS_NOTARY_TEAM_ID",
            "AURA_MACOS_NOTARY_PASSWORD",
        ][..],
        Platform::Windows => &[
            "AURA_WINDOWS_SIGNTOOL_CERT_PATH",
            "AURA_WINDOWS_SIGNTOOL_CERT_PASSWORD",
            "AURA_WINDOWS_TIMESTAMP_URL",
        ][..],
    };
    ...
    match fs::read_to_string(&workflow_path) {
        Ok(text)
            if text.contains("release-readiness")
                ...
                && text.contains("aura-release-packages-*")
                && text.contains("aura-release-binaries-*") =>
        {
            report.pass(
                "release workflow",
...
mod tests {
    ...
    fn signing_env_missing_is_platform_specific() {
        ...
        assert!(macos_missing.contains(&"AURA_MACOS_CODESIGN_IDENTITY"));
        let windows_missing = signing_env_missing(Platform::Windows);
        assert!(windows_missing.contains(&"AURA_WINDOWS_SIGNTOOL_CERT_PATH"));
    }
