...
library crate 保持 `default-features = false`；最终 app crate 通过 target-specific dependencies 在 `gpui` 与 `gpui_platform` 上启用平台 feature。禁止使用 `open-gpui` 等重命名或社区 fork，也不要发布包含本地 path GPUI 依赖或 `[patch]` override 的 Liora crate。
