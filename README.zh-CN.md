...
| 应用一条依赖接入 | `cargo add liora` 暴露稳定的 SDK facade：core、theme、components、icons、tray、packaging 与 updater helpers。 |
...
- **更新流水线**：`liora-updater` 检查 GitHub Releases，选择当前平台的 Docs/Gallery 资产，下载到缓存，校验 `SHA256SUMS.txt`，并返回明确的安装计划。
...
只有在工作区需要更窄依赖面时，才直接依赖 `liora-components` 或 `liora-packager` 等底层 crate。

### Updater 模块

`liora-updater` 已包含在默认的 `liora` facade 中，也可以单独依赖：

```bash
cargo add liora-updater
```

它提供 GitHub Release 更新检查、按平台选择资产、缓存下载、基于 `SHA256SUMS.txt` 的 SHA-256 校验，以及显式安装计划。Gallery 和 Docs 会在 About 栏/启动流程自动检查新版本，并把通过校验的资产下载到更新缓存；安装仍保持为可见用户动作，因为部分安装器需要系统提权或替换正在运行的可执行文件。
...
发布到 crates.io 的 Liora SDK crates 直接依赖 Zed 官方 crates.io 包 `gpui = "0.2.2"`（`repository = "https://github.com/zed-industries/zed"`）。它们不能使用重命名 fork 或社区 fork，也不会 vendor、发布或强制使用本仓库 `third_party/zed` patch。

Gallery 与 Docs 默认也按同一策略构建：使用官方 crates.io `gpui`，并只在最终 app crate 中开启平台 feature。`third_party/zed` 仅作为 Linux 启动窗口 patch 讨论的未发布上游源码对照材料保留。如需验证该本地 patch，只能在临时分支、应用 root 中使用：
...
不要把这个 override 写进任何可发布的 Liora crate manifest，也不要发布包含任意 `[patch]` 或 path GPUI 依赖的 Liora crate。下游产品也应使用 Zed 官方 `gpui`；只有在自己维护应用级验证分支时才临时切到上游源码 patch。
