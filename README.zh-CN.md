...
| 应用一条依赖接入 | `cargo add liora` 暴露稳定的 SDK facade：core、theme、components、icons、tray 与 packaging helpers。 |
...
发布到 crates.io 的 Liora SDK crates 明确依赖 crates.io `open-gpui` / `open-gpui-platform`。它们不会 vendor、发布或强制使用本仓库 `third_party/zed` patch，因为 GPUI backend override 应由最终应用的 root manifest 决定。

Gallery 与 Docs 默认也按同一套 `open-gpui` 依赖构建。只有在本地验证 Linux 启动窗口首帧状态时，才使用 `third_party/zed` 中保留的最小 Zed 源码 patch。验证方式是在临时分支中临时修改根 `[workspace.dependencies]` alias，让所有 Liora crates 一起使用 Zed git `gpui` / `gpui_platform`，然后在应用 root 增加 git source patch：
...
不要把这个 override 写进任何可发布的 Liora crate manifest。`third_party/zed` 里的包名是 upstream `gpui` / `gpui_linux`，不能直接 patch crates.io 上名为 `open-gpui` / `open-gpui-linux` 的包；如果应用仍使用 `open-gpui`，需要使用包名一致的私有 fork。
