...
Liora 的 GPUI 相关 SDK crate 是 git-only：它们依赖官方 Zed GPUI 仓库中比 crates.io 旧版 GPUI 更新很多的固定 revision。应用侧应把 Liora 与 GPUI 一起 pin 住：
...
只有在工作区需要更窄依赖面时，才以 git/path 方式直接依赖 `liora-components` 等底层 crate。`liora-packager` 和 `liora-updater` 是独立工具 crate，仍可从 crates.io 使用。
...
Liora 依赖 Zed 官方上游仓库，并用 commit pin 住以保证构建可复现：
...
library crate 保持 `default-features = false`；最终 app crate 通过 target-specific dependencies 在 `gpui` 与 `gpui_platform` 上启用平台 feature。禁止使用 `open-gpui` 等重命名或社区 fork，也不要给 GPUI 依赖添加 `version` 回退。crates.io 不能发布依赖 git-only GPUI 源的包，因此 GPUI 相关 Liora SDK crate 明确 `publish = false`；应用应使用 git `rev` 依赖 Liora，并同时依赖匹配的 Zed 官方 GPUI `rev`。
