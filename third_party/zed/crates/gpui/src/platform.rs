...
pub struct WindowParams {
    ...
    /// Initial platform window state requested before the first map/configure.
    pub initial_window_state: InitialWindowState,
...
/// Initial state requested from the platform before the first map/configure.
#[derive(Debug, Copy, Clone, PartialEq)]
pub enum InitialWindowState {
    /// Open as a normal window.
    Windowed,
    /// Ask the platform/compositor to open maximized before first display.
    Maximized,
    /// Ask the platform/compositor to open fullscreen before first display.
    Fullscreen,
}
