...
- CI
  ...
  - CI 真实反馈修正：`cargo-generate-rpm --metadata-overwrite` 必须使用 `GenerateRpm.<app>.toml#package.metadata.generate-rpm` 分支加载，否则 runner 会报 `Missing field: package.assets`。
