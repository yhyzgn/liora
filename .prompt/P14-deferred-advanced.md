...
1. `Tour`：需要 overlay/position/highlight，依赖 Popover/Portal。
2. `VirtualizedTable` / `VirtualizedTree`：性能型组件，最后集中做。

## 验收命令

```bash
cargo check -p aura-components -p aura-gallery -p aura-docs --bin check_snippets
cargo test -p aura-components carousel && cargo test -p aura-components calendar && cargo test -p aura-components input_tag && cargo test -p aura-components mention && cargo test -p aura-components watermark && cargo test -p aura-components tree_select
