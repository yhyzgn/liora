...
1. `VirtualizedTable` / `VirtualizedTree`：性能型组件，最后集中做。

## 验收命令

```bash
cargo check -p aura-components -p aura-gallery -p aura-docs --bin check_snippets
cargo test -p aura-components carousel && cargo test -p aura-components calendar && cargo test -p aura-components input_tag && cargo test -p aura-components mention && cargo test -p aura-components watermark && cargo test -p aura-components tree_select && cargo test -p aura-components tour
...
- `VirtualizedTable`: 新增大数据虚拟表格，支持 `TableColumn` 列定义、固定表头、`ListState` 可见区行渲染、Aura `VirtualScrollbar`、高度/行高/overdraw、斑马纹/边框/加载/空状态、三态排序回调。
...
- `VirtualizedTree`: 新增大型树虚拟化控件，维护原始 `TreeNode`、展开 key、选择 key 和轻量可见节点元数据，使用 `ListState` 只渲染可见行，支持默认展开/选中、展开/折叠、单选/多选、checkbox 风格、on_node_click 回调、高度/行高/缩进/overdraw 配置和 Aura 滚动条。
