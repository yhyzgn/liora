...
impl Tree {
    ...
    pub fn on_node_click(mut self, f: impl Fn(SharedString, &mut Window, &mut App) + 'static) -> Self {
    ...
    fn render_node(&self, node: &TreeNode, depth: u32, theme: &aura_theme::Theme, cx: &Context<Self>) -> AnyElement {
        ...
        div().flex().flex_col()
            .child(
                div()
                    ...
                    .h(px(32.0)).pl(padding_left).pr_4()
                    .text_color(if is_selected { theme.primary.base } else { theme.neutral.text_1 })
                    .bg(if is_selected { theme.primary.base.opacity(0.1) } else { gpui::transparent_black() })
                    .hover(|s| s.bg(theme.neutral.hover))
                    .child(
                        // Expand Icon
                        div().flex().items_center().justify_center().w(px(20.0))
                            .id(format!("expand-{}", id.clone()))
                            .when(has_children, |s| s
                                ...
                                .child(Icon::new(if is_expanded { IconName::ChevronDown } else { IconName::ChevronRight })
                                    .size(px(14.0)).color(theme.neutral.text_3))
                            )
                    )
                    .child(
                        div().flex_1().id(format!("content-{}", id.clone())).on_click(cx.listener({
                            let id = id.clone();
            ...
            .when(is_expanded && has_children, |s| s.children(
                node.children.iter().map(|child| self.render_node(child, depth + 1, theme, cx))
            ))
            .into_any_element()
    }
}
...
impl Render for Tree {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .children(self.data.iter().map(|node| self.render_node(node, 0, &theme, cx)))
    }
}
