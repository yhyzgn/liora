#!/usr/bin/env bash
# Sync Ant Design SVG icons from the official upstream repository.
#
#   ./scripts/sync-antd.sh         # incremental cache refresh
#   ./scripts/sync-antd.sh --full  # force overwriting copied SVGs

set -euo pipefail

UPSTREAM_REPO='https://github.com/ant-design/ant-design-icons.git'
UPSTREAM_PATH='packages/icons-svg/svg'
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CRATE_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
PROJECT_DIR="$(cd "$CRATE_DIR/../.." && pwd)"
CACHE_DIR="$PROJECT_DIR/target/antd-icons-cache"
...
python3 "$SCRIPT_DIR/sync_antd.py" "$CACHE_DIR/$UPSTREAM_PATH" "$SVG_DIR" "$FULL"
