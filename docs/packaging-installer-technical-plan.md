...
适用范围：`liora-gallery`、`liora-docs` 以及后续所有纯 GPUI Liora 主程序

## 1. 目标

为 Liora 的 GPUI 原生桌面程序建立一套可持续维护的跨平台安装器与发布产物构建体系，覆盖 Linux、macOS、Windows 的主流安装格式，并能在 CI 中自动构建、校验、归档、上传发布产物。

本阶段的核心目标不是改变应用运行时，而是补齐 **构建、打包、安装、卸载、桌面集成、签名预留、发布归档** 能力。

## 2. 绝对约束

- Liora 仍然是 **纯 Rust + GPUI 原生应用**。
- 严禁把 `liora-gallery`、`liora-docs` 或后续主程序改造成 Tauri 应用。
- 严禁引入 WebView、HTML/CSS/DOM、浏览器运行时、前端构建链作为应用主体。
- 可以参考 Tauri 的打包格式与 bundler 思路，但只能使用与运行时解耦的打包工具，不引入 Tauri Runtime。
- 打包配置必须面向多个 Liora app 复用，不能只为单个二进制硬编码。
...
Zed 的优点是控制力强，缺点是脚本复杂度高，不适合 Liora 初期直接全量复制。
...
Zedis 是更接近 Liora 当前体量的 GPUI 应用。它主要使用：
...
Zedis 的方式简单，适合作为初期参考，但格式覆盖不够完整，不能单独满足 Liora 的全平台安装器目标。
...
但完整 Tauri 是 WebView 应用框架，不适合 Liora。本项目只参考其打包产物与配置思想，不采用 Tauri Runtime。

### 3.4 Cargo Packager

`cargo-packager` 是当前最符合 Liora 需求的主工具：它是纯打包工具和库，用于给已有可执行文件生成安装器或 app bundle，不要求应用使用 Tauri Runtime。
...
采用 **`liora-packager` 领域逻辑库 + `xtask` 编排 + `cargo-packager` 主打包 + 平台补充工具** 的混合方案。

### 4.0 命名决策

内部模块命名确定为 **`liora-packager`**，不使用 `liora-installer`。
...
| 模块 | 职责 |
|---|---|
| `crates/liora-packager` | 可测试的打包领域逻辑库：app metadata、format enum、manifest、checksum、资源校验 |
...
| 层级 | 工具 | 责任 |
|---|---|---|
| 打包领域逻辑 | `crates/liora-packager` | app metadata、format enum、manifest、checksum、配置校验、平台资源校验 |
...
| Linux | portable `.tar.gz` | 中立解压运行包，Liora supplemental backend 已接入 |
...
packaging/
  ...
  icons/
    ...
    liora-docs.ico
  linux/
    liora-gallery.desktop
    liora-docs.desktop
    liora-gallery.metainfo.xml
    liora-docs.metainfo.xml
    rpm/
      liora-gallery.toml
      liora-docs.toml
  macos/
...
- `liora-packager` 是可测试、可复用的打包领域逻辑库，不直接作为最终用户命令入口。
- `xtask` 是唯一对外构建入口。
...
```text
id: dev.liora.Gallery / dev.liora.Docs
name: Liora Gallery / Liora Docs
binary: liora-gallery / liora-docs
version: workspace/app Cargo.toml version
vendor: Liora
category: DeveloperTool / Documentation
short_description: Native GPUI component gallery / Native Liora documentation app
long_description: ...
icons: png/icns/ico
...
| App | App ID | Binary |
|---|---|---|
| Liora Gallery | `dev.liora.Gallery` | `liora-gallery` |
| Liora Docs | `dev.liora.Docs` | `liora-docs` |

如果未来有公司/组织域名，应统一替换为正式反向域名，例如 `cn.recycloud.liora.Gallery`。
...
```text
liora-gallery-<version>-linux-x86_64.AppImage
liora-gallery-<version>-linux-x86_64.deb
liora-gallery-<version>-linux-x86_64.rpm
liora-gallery-<version>-macos-aarch64.dmg
liora-gallery-<version>-windows-x86_64-setup.exe
liora-gallery-<version>-windows-x86_64.msi
...
- [x] 新增 `crates/liora-packager` crate。
- [x] 新增 `xtask` crate。
...
- [x] portable tar.gz（Liora supplemental backend 已接入，不再映射为 cargo-packager pacman）。
...
- [x] tag push 触发 release build（`v*` tags），并校验 tag 为 `vX.Y.Z` 且匹配 `crates/liora-packager` version。
...
- 不引入 Tauri Runtime。
- `liora-gallery`、`liora-docs` 都能生成平台安装包。
...
- `cargo xtask package --app <gallery|docs> --format <fmt> --dry-run --skip-build` 会生成 `target/liora-packager/Packager.<app>.toml`，并打印实际 `cargo packager ...` 调用。
...
- `tar.gz` 走 Liora supplemental backend：收集 release binary、icons、Linux desktop/metainfo、README 和启动脚本，输出 `<package>-<version>-<platform>-<target-triple>.tar.gz`。
...
- Release tag validation 已补：`v*` tag 必须是 `vX.Y.Z` 且匹配 `crates/liora-packager` version，防止 MSI/Release 使用非法或错配版本。
- 后续仍需在真实 `v*` tag 上验证 release job、GitHub Release asset 上传、Windows MSI，以及签名/公证和真正安装/卸载 smoke。

## 15. 决策记录

- 采用 `liora-packager` 作为 Liora 内部打包领域逻辑库。
- 采用 `cargo-packager` 作为主打包工具。
- 采用 `xtask` 作为统一编排入口，命令仍为 `cargo xtask package ...`，不直接要求用户运行 `cargo run -p liora-packager`。
- RPM 使用补充工具实现，不阻塞主路径。
...
- `LICENSE.md` 明确当前 `LicenseRef-Liora` 策略，避免无意宣称 OSS license。
...
- `.github/workflows/package.yml` 在打包前运行 readiness gate；`v*` tag release 自动启用 `LIORA_REQUIRE_SIGNING=true`，没有签名 secret 时会阻止正式发布。
