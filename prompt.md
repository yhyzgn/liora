...
- GitHub Actions 在 `v*` tag 上自动下载各平台 artifacts，基于 git commits 自动收集 changelog，创建/更新 GitHub Release 并上传全部构建产物。
...
5. **GitHub Release automation 后续增强**：基础自动 Release 已接入；后续可补 release draft/prerelease 策略、artifact 命名清洗、按平台聚合 manifest。
