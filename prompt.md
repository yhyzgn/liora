...
- P10 是原生统计图阶段：开发纯原生 GPUI 统计图控件，参考 GPUI 官方 canvas/path/paint API；`vicanso/zedis` 只能作为案例参考，不复制其依赖或实现结构。当前 LineChart 与 AreaChart Overlay 已支持原生 hover tooltip / 最近点 hit testing。
...
- P14 是延后高级组件补全阶段：从 P9 backlog 迁移而来，Wave 1 已完成 Carousel、Calendar、InputTag；Wave 2 已完成 Mention、Watermark；Wave 3 已完成 TreeSelect；Wave 4 已完成 Tour；Wave 5 已完成 VirtualizedTable；Wave 6 已完成 VirtualizedTree；P14 已完成。详见 `.prompt/P14-deferred-advanced.md`。
...
3. **Install / uninstall smoke**：已补 artifact smoke 与 `xtask package install-smoke` plan-only gate；portable `.tar.gz` 支持显式 `--execute-install` 安全解压/删除验证。仍待补真正系统级 deb/rpm/AppImage/macOS/Windows 安装/卸载执行 gate。
